...
for (const method of ["error", "guarded"]) test(`live model analyzes the actual ${method} interaction`, async ({ page }) => {
  ...
  await page.getByRole("button", { name: "임시저장 필터" }).click();
  if (method === "guarded") await page.getByRole("button", { name: "빈 응답 기본값 처리", exact: true }).click();
  await page.getByRole("button", { name: "새로고침 실행", exact: true }).click();
  await page.getByRole("button", { name: "방금 오류 찾아줘" }).click({ timeout: 15_000 });
  ...
  expect(result).toMatch(method === "guarded" ? /기본값|빈|오류/ : /undefined|map|TypeError/);
