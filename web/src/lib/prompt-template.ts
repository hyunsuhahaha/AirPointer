...
export type PromptTemplate = {
  wrapperIntro: string;
  wrapperOutro: string;
  contextByKind: Record<CaptureKind, string>;
...
export const DEFAULT_PROMPT_TEMPLATE: PromptTemplate = {
  wrapperIntro: "AirPointer가 사용자가 확정한 화면과 질문을 보냈습니다.",
  wrapperOutro: "첨부 화면을 확인하고 위 요청에 답하세요.",
  contextByKind: {
    screenshot: "현재 화면을 캡처했습니다.",
    region: "선택한 화면 영역을 캡처했습니다.",
    replay: "제스처 완료 시점 기준 최근 화면 기록입니다. 첨부 이미지는 과거에서 현재 순서입니다.",
  },
...
function mergeTemplate(partial: Partial<PromptTemplate>): PromptTemplate {
  return {
    wrapperIntro: partial.wrapperIntro || DEFAULT_PROMPT_TEMPLATE.wrapperIntro,
    wrapperOutro: partial.wrapperOutro || DEFAULT_PROMPT_TEMPLATE.wrapperOutro,
    contextByKind: { ...DEFAULT_PROMPT_TEMPLATE.contextByKind, ...partial.contextByKind },
