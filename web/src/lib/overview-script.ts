...
// AI pops up and delivers its line in a speech bubble.
export const AI_BEAT = (() => {
  const start = STAGES[0].start + STAGES[0].duration;
  return { start, clockEnd: start + 1, popAt: start + 1.05, bubbleAt: start + 1.4, end: STAGES[0].end };
})();
