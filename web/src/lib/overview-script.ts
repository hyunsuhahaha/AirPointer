...
// `hold` is a pause (in real seconds) after the beat finishes. RANT_SPEED
// scales every beat's typing time. The last two beats keep the flat top
// speed of "logs" so the flood never stalls before the blackout.
const RANT_SPEED = 1.43;
const AI_BEAT_SECONDS = 4.3;
type StageSpec = { id: string; endsAfter: string; duration: number; hold: number; ease: number };
const STAGE_SPECS: StageSpec[] = [
  { id: "intro", endsAfter: OPENING, duration: 7.5, hold: AI_BEAT_SECONDS, ease: 1 },
  ...
  { id: "gone", endsAfter: "모니터만 멍하니 바라보다가 ", duration: 0.57, hold: 0, ease: 1 },
  { id: "spiral", endsAfter: RANT, duration: 1.14, hold: 0.8, ease: 1 },
];
...
export const STAGES: Stage[] = (() => {
  ...
  return STAGE_SPECS.map((spec) => {
    const to = spec.endsAfter === RANT ? RANT.length : spec.endsAfter === OPENING ? OPENING.length : RANT.indexOf(spec.endsAfter, from) + spec.endsAfter.length;
    if (to <= from) throw new Error(`overview stage "${spec.id}" boundary not found`);
    const duration = spec.duration / RANT_SPEED;
    const stage = { ...spec, duration, from, to, start, end: start + duration + spec.hold };
    from = to;
    start = stage.end;
    return stage;
  });
...
// The pause after the opening: a fast-forwarded "15분 검토" clock, then the
// AI pops up and delivers its line in a speech bubble, then a smug follow-up.
export const AI_BEAT = (() => {
  const start = STAGES[0].start + STAGES[0].duration;
  return { start, clockEnd: start + 1, popAt: start + 1.05, bubbleAt: start + 1.4, followupAt: start + 2.5, end: STAGES[0].end };
})();
