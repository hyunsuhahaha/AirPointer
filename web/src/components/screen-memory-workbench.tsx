...
import { Archive, BookmarkSimple, Brain, Bug, Check, ClockCounterClockwise, Copy, DownloadSimple, FileText, MagnifyingGlass, Play, Plus, Tag, Trash, TrendUp } from "@phosphor-icons/react";
...
type View = "search" | "timeline" | "bookmarks" | "summary" | "reports";
...
export function ScreenMemoryWorkbench({ recording, onAddToReplay }: {
  recording: boolean;
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      await saveScreenMemoryReport({ from: start, to, title: "자동 개발 리포트", markdown: buildDeveloperReport(reportFrames, start, to, "자동 개발 리포트"), automatic: true });
  ...
  const makeReport = async () => {
    ...
    const to = Date.now();
    await saveScreenMemoryReport({ from, to, title: "개발 화면 리포트", markdown: buildDeveloperReport(reportFrames, from, to, "개발 화면 리포트"), automatic: false });
  ...
  const download = (report: ScreenMemoryReport) => {
    ...
    const anchor = document.createElement("a"); anchor.href = url; anchor.download = `whatwas-report-${new Date(report.createdAt).toISOString().slice(0, 10)}.md`; anchor.click(); URL.revokeObjectURL(url);
  };
  ...
  return <section className={styles.workbench} id="memory" aria-labelledby="memory-title">
    ...
    {!frames.length && view !== "reports" ? <div className={styles.empty}>
    ...
    {frames.length > 0 && view === "search" && <div className={styles.searchView}>
      ...
      <div className={styles.resultGrid}>{frames.map((frame) => <MemoryCard key={frame.id} frame={frame} selected={selectedId === frame.id} onSelect={() => setSelectedId(frame.id)} onBookmark={async () => { const bookmarked = !frame.bookmarked; await updateScreenMemoryFrame(frame.id, { bookmarked }); }} />)}</div>
    </div>}

    {frames.length > 0 && view === "timeline" && <Timeline frames={frames} selectedId={selectedId} onSelect={setSelectedId} onBookmark={async (frame) => { await updateScreenMemoryFrame(frame.id, { bookmarked: true }); onAddToReplay(frame); setMessage("이 시점을 영구 북마크하고 다음 리플레이에도 추가했습니다."); }} />}
    ...
    {frames.length > 0 && view === "bookmarks" && <div className={styles.bookmarkView}>
      ...
      {selected?.bookmarked ? <BookmarkEditor key={selected.id} frame={selected} onAddToReplay={onAddToReplay} onMessage={setMessage} /> : <div className={styles.noBookmarks}><BookmarkSimple size={30} /><p>북마크가 없습니다. 검색이나 시간여행에서 중요한 화면을 북마크하세요.</p></div>}
    </div>}
...
function BookmarkEditor({ frame, onAddToReplay, onMessage }: {
  ...
  return <div className={styles.bookmarkEditor}>
    ...
    <div>
      ...
      <button type="button" className={styles.danger} onClick={async () => { await deleteScreenMemoryFrame(frame.id); }}><Trash size={15} />삭제</button>
    </div>
  </div>;
}
