...
def test_ui_fits_and_draws_selection(tmp_path: Path) -> None:
    app = App()
    try:
        app.root.update()
...
def test_screenshot_capture_also_opens_prompt(tmp_path: Path) -> None:
    # Screenshot used to send instantly with a default question; the prompt window is now shared across every
