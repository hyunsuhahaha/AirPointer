...
## 브라우저 PiP 연결 시 확인한 수정 (2026-09-07)

브라우저 미리보기 감지가 `observe(video, 1, 1, ...)`를 호출하면서 정규화된 좌표를
얻으려 했으나, 이 크기가 `drawImage`의 원본 영역 크기에도 쓰여 왼쪽 위 한 픽셀만
샘플링하고 있었다. 이제 `videoWidth`/`videoHeight`로 전체 화면을 비교한 뒤 감지된
사각형 좌표만 0~1로 정규화한다. 따라서 화면 가운데의 알림 같은 변화도 감지한다.

변화 전후 카드의 이전 이미지는 최초 변화 시점보다 **앞선 마지막 프레임**을 고른다.
최초 변화 시점과 가장 가까운 프레임을 고르면 이전·이후에 같은 이미지가 나올 수 있었다.
`web/tests/browser-highlight.test.ts`는 전체 원본 크기, 정규화한 국소 영역, 서로 다른
변화 전후 프레임을 검사한다. 이 결과는 메인 페이지와 PiP의 변화 전후 표시가 공유한다.
